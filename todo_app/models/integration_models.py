...
class ThirdPartyAppIntegration(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='app_integrations')
    third_party_app = models.ForeignKey(ThirdPartyApp, on_delete=models.CASCADE)
...
class ThirdPartyEntityMapping(models.Model):
    ...
    integration = models.ForeignKey(ThirdPartyAppIntegration, on_delete=models.CASCADE)
    ...
    def create_mapping(trackable_entity, third_party_id, integration):
        ...
        if ThirdPartyEntityMapping.get_trackable_entity(third_party_id, integration):
            return
        content_type = ContentType.objects.get_for_model(type(trackable_entity))
        logger.info(f'\n\nMap creating: {trackable_entity.title}, todoist_id {third_party_id}\n\n')
        ...
        logger.info(f'\n\nMap created: {mapping.trackable_entity.title}, todoist_id {mapping.third_party_id}\n\n')
        return mapping
